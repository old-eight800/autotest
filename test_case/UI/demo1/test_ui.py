...
test_case_path = os.path.join(BASE_DIR,"test_case","UI", "demo1","test_demo1_case.yaml")
with open(test_case_path, "r", encoding="utf-8") as file:
    case_dict = yaml.load(file.read(), Loader=yaml.FullLoader)

def test_case(demo1cases, page: Page):
    caseno = list(demo1cases.keys())[0]
    descrption = dict(list(demo1cases.values())[0])["descrption"]
    test_step = list(demo1cases.values())[0]["test_step"]
    expect_result = dict(list(demo1cases.values())[0])["expect_result"]
    allure_title(caseno)
